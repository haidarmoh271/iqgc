...
function nextQuestion(code) {
  ...
  // التايمر يبدأ بعد 3 ثواني — أعلم كل عميل مسبقاً بالـ timestamp الدقيق
  room.timerStartTime    = room.questionStartTime + 3000;

  const q     = room.quiz.questions[room.currentQ];
  const total = room.quiz.questions.length;
  const timerStartsAt = room.timerStartTime; // نقطة مرجعية واحدة للجميع

  log.event('question:start', { code, q: room.currentQ + 1, total, timerStartsAt });

  // ── أرسل السؤال + timerStartsAt للجميع فوراً ──
  io.to(room.host).emit('host:question', {
    ...
    timerStartsAt, // الهوست يعرف متى سيبدأ التايمر
  });
  ...
  io.to(code).emit('game:question', {
    ...
    timerStartsAt, // اللاعب يعرف متى سيبدأ التايمر
  });

  // ── بعد 3 ثواني: أرسل تأكيد للشاشة الكبيرة فقط (تحتاج syncDisplay) ──
  setTimeout(() => {
    if (!rooms[code] || rooms[code].state !== 'question') return;
    io.to('display:' + code).emit('display:sync', {
      action: 'timerStart',
      data: { startedAt: timerStartsAt, time: q.time },
    });
  }, 3000);
}
...
function showResults(code) {
  ...
  // speedBonus — أي شخص أجاب خلال أول 5 ثواني (صح أو غلط)
  const SPEED_WINDOW_MS = 5000;
  sorted
    .filter(e => e.elapsed <= SPEED_WINDOW_MS) // أول 5 ثواني بغض النظر عن الصحة
    .slice(0, 5)
    .forEach(entry => {
      const player = room.players[entry.socketId];
      if (!player || player.answers[qIdx] == null) return;
      if (player.answers[qIdx].speedBonus !== 0) return; // idempotent — لا تُضاف مرتين
  ...
  Object.entries(room.players).forEach(([sid, player]) => {
    ...
    io.to(sid).emit('player:answerResult', {
      ...
      correctPts:   ans ? ans.correctPoints : 0,
      speedBonus:   ans ? ans.speedBonus  : 0,
      streakBonus:  ans ? ans.streakBonus : 0,
...
io.on('connection', socket => {
  ...
  socket.on('display:join', safeHandler(socket, 'display:join', ({ code }) => {
    ...
    if (room.state === 'question') {
      ...
      currentState.questionData = {
        ...
        time: q.time, image: q.image || null,
        elapsed: elapsedSec,
        timerStartsAt: room.timerStartTime || null, // الشاشة العائدة تبدأ التايمر من هنا
      };
    ...
    if (playerId) {
      ...
      if (existing) {
        ...
        if (room.state === 'question') {
          ...
          // للاعب العائد: أرسل السؤال + وقت بدء التايمر الحقيقي مباشرةً
          // بدل elapsed: يحسب العميل الوقت من Date.now() - startedAt
          socket.emit('game:question', {
            index: room.currentQ, total: room.quiz.questions.length,
            question: q.question, answers: q.answers,
            time: q.time, image: q.image || null,
            elapsed: 0,
            timerStartsAt: room.timerStartTime, // اللاعب العائد يحسب المتبقي مباشرة
          });
        }
        io.to(room.host).emit('host:playerList', { players: getPlayerList(room) });
  ...
  socket.on('player:answer', safeHandler(socket, 'player:answer', ({ code, answerIndex }) => {
    ...
    // anti-bot: احسب من timerStartTime (وقت البدء الحقيقي بعد 3-2-1)
    // إذا لم يصل timerStart بعد (نافذة <3s من السؤال) ارفض الإجابة
    const timerRef = room.timerStartTime || (room.questionStartTime + 3000);
    const elapsed = Date.now() - timerRef;
    if (elapsed < 100) return; // anti-bot: استجابة قبل بدء التايمر الحقيقي

    if (!room.answerTimes[room.currentQ]) room.answerTimes[room.currentQ] = [];
    // elapsed من وقت بدء التايمر الحقيقي (لحساب speed bonus بشكل صحيح)
    const timerElapsed = Date.now() - (room.timerStartTime || timerRef);
    room.answerTimes[room.currentQ].push({ name: player.name, elapsed: timerElapsed, correct, socketId: socket.id });

    // streak: يزيد فقط عند الإجابة الصحيحة، يُصفَّر عند الخطأ
    if (correct) {
    ...
    const streakBonus   = (correct && player.streak >= 3) ? (player.streak - 2) * 10 * dp : 0;
