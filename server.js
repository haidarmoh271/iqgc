...
function nextQuestion(code) {
  ...
  room.state             = 'question';
  room.questionStartTime = Date.now(); // وقت إرسال السؤال (للمرجع فقط)
  room.timerStartTime    = null;       // يُعيَّن بعد 3 ثواني (عداد 3-2-1)
  ...
  // ── بعد 3 ثواني (نهاية عداد 3-2-1): سجّل وقت بدء التايمر الحقيقي وأبثّه ──
  setTimeout(() => {
    if (!rooms[code] || rooms[code].state !== 'question') return;
    rooms[code].timerStartTime = Date.now();
    const startedAt = rooms[code].timerStartTime;

    // أرسل للاعبين
    io.to(code).emit('game:timerStart', { startedAt, time: q.time });
    // أرسل للهوست
    io.to(rooms[code].host).emit('host:timerStart', { startedAt, time: q.time });
    // أرسل لشاشة العرض عبر syncDisplay
    io.to('display:' + code).emit('display:sync', {
      action: 'timerStart',
      data: { startedAt, time: q.time },
    });
    log.event('timer:start', { code, q: rooms[code].currentQ + 1, startedAt });
  }, 3000);
}
  ...
  socket.on('display:join', safeHandler(socket, 'display:join', ({ code }) => {
    ...
    if (room.state === 'question') {
      ...
      currentState.questionData = {
        ...
        time: q.time, image: q.image || null,
        elapsed: elapsedSec,
        timerStartedAt: room.timerStartTime || null, // وقت البدء الحقيقي
      };
      ...
      if (existing) {
        ...
        if (room.state === 'question') {
          ...
          socket.emit('game:question', {
            ...
            question: q.question, answers: q.answers,
            time: q.time, image: q.image || null,
            elapsed: 0, // سنرسل timerStart منفصلاً إذا بدأ التايمر
          });
          // إذا التايمر بدأ فعلاً — أرسل startedAt للعميل ليحسب بنفسه
          if (room.timerStartTime) {
            setTimeout(() => {
              socket.emit('game:timerStart', {
                startedAt: room.timerStartTime,
                time: q.time,
              });
            }, 100);
          }
